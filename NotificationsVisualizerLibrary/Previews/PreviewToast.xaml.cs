...
namespace NotificationsVisualizerLibrary
{
    ...
    public sealed partial class PreviewToast : UserControl, IPreviewToast
    {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public JsonParseToastResult InitializeFromJson(string json, PreviewNotificationData data)
        {
            JsonParseToastResult result = JsonParser.ParseToast(json, CurrFeatureSet);
            ...
            if (result.IsOkForRender())
            {
                ...
                if (result.IsOkForRender())
                {
                    InitializeContent(result.Content, result);
                }
            }
        ...
        private AdaptiveCards.Rendering.Uwp.IAdaptiveActionElement _selectAction;
        private Dictionary<string, FrameworkElement> _elementsWithIds;

        private IToast _currContent;
        private JsonToastContent _currJsonContent;
        private string _finalCardJson;
#if CARDS
        private AdaptiveCards.Rendering.Uwp.RenderedAdaptiveCard _renderedAdaptiveCard;
#endif
        ...
        {
            HasContent = false;
            _currJsonContent = null;
            _finalCardJson = null;
#if CARDS
            _renderedAdaptiveCard = null;
#endif
            ...
            ContentAdaptiveJson.Child = null;
            ...
            if (CurrFeatureSet.RS1_Style_Toasts)
            {
                GridAppLogo.Width = 16;
                GridAppLogo.Height = 16;
            }

            else
            {
                GridAppLogo.Width = 32;
                GridAppLogo.Height = 32;
            }

            if (CurrFeatureSet.RS3_Style_Toasts)
            {
                GridAppLogo.Margin = new Thickness(16, 22, 16, 0);
            }
            else
            {
                GridAppLogo.Margin = new Thickness(12);
            }
...
#if CARDS
        private void InitializeContent(JsonToastContent toastContent, JsonParseToastResult result)
        {
            ResetContent();
            _currJsonContent = toastContent;

            if (toastContent != null)
            {
                // If title not provided, falls back to subtitle
                var title = toastContent.Title ?? toastContent.Subtitle;

                if (title != null)
                {
                    TextBlockTitle.Text = title.Text;
                    TextBlockTitle.MaxLines = title.MaxLines.GetValueOrDefault(2);
                    TextBlockTitle.TextWrapping = TextWrapping.WrapWholeWords;

                    if (toastContent.Subtitle != null && toastContent.Subtitle != title)
                    {
                        TextBlockBody.Text = toastContent.Subtitle.Text;
                        TextBlockBody.MaxLines = Math.Min(toastContent.Subtitle.MaxLines.GetValueOrDefault(4), 4);
                        TextBlockBody.TextWrapping = TextWrapping.WrapWholeWords;
                ...
                // Hero image
                if (toastContent.Images?.Card?.Url != null)
                {
                    ImageHero.Visibility = Visibility.Visible;
                    ImageHeroBrush.ImageSource = ImageHelper.GetBitmap(toastContent.Images.Card.Url);
                }
                ...
                // App logo override
                bool hasAppLogo = false;
                if (toastContent.Images?.Profile?.Url != null)
                {
                    hasAppLogo = true;
                    DefaultImageAppLogo.Visibility = Visibility.Collapsed;

                    var source = ImageHelper.GetBitmap(toastContent.Images.Profile.Url);

                    if (toastContent.Images.Profile.Style == JsonToastAppLogoImageStyle.Person)
                    {
                    ...
                    // AppLogoOverride images will become 48px large in RS1
                    GridAppLogo.Width = 48;
                    GridAppLogo.Height = 48;
                    ...
                    // The app logo gets pushed down a bit more in RS3 and RS4
                    GridAppLogo.Margin = new Thickness(16, 24, 16, 24);
                }

                // Header
                if (toastContent.MsHeader != null)
                {
                    if (hasAppLogo)
                    {
                        TextBlockHeaderWhenAppLogoPresent.Text = toastContent.MsHeader.Title;
                    }
                    else
                    {
                        TextBlockHeaderWhenAppLogoNotPresent.Text = toastContent.MsHeader.Title;
                    }
                }

                // Attribution
                if (toastContent.MsAttributionText != null)
                {
                    StackPanelAttribution.Visibility = Visibility.Visible;
                    ...
                    TextBlockAttributionSecondPart.Text = toastContent.MsAttributionText.Text;
                }

                if (toastContent.Card != null)
                {
                    try
                    {
                        _selectAction = toastContent.Card.SelectAction;

                        // Clear the select action since we use it for the entire toast and will handle it ourselves
                        toastContent.Card.SelectAction = null;
                    }
                    catch (Exception ex)
                    {
                        result.Errors.Add(new ParseError(ParseErrorType.Warning, "Top-level selectAction issue: " + ex.ToString()));
                    }

                    try
                    {
                        if (toastContent.Card.Body.Count > 0 || toastContent.Card.Actions.Count > 0 || !string.IsNullOrWhiteSpace(toastContent.Card.BackgroundImage))
                        {
                            var renderedAdaptiveCard = CardRenderer.RenderCard(toastContent.Card);
                            foreach (var e in renderedAdaptiveCard.Errors)
                            {
                                result.Errors.Add(new ParseError(ParseErrorType.Warning, e.Message));
                            }
                            foreach (var w in renderedAdaptiveCard.Warnings)
                            {
                                result.Errors.Add(new ParseError(ParseErrorType.Warning, w.Message));
                            }
                            if (renderedAdaptiveCard.FrameworkElement != null)
                            {
                                ContentAdaptiveJson.Child = new Border()
                                {
                                    Child = renderedAdaptiveCard.FrameworkElement
                                };
                                _renderedAdaptiveCard = renderedAdaptiveCard;
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        result.Errors.Add(new ParseError(ParseErrorType.Warning, ex.ToString()));
                    }
                }
            }
        ...
        private void TriggerCardActivation(AdaptiveCards.Rendering.Uwp.IAdaptiveActionElement action)
        {
            if (action == null)
            {
                return;
            ...
            if (action is AdaptiveCards.Rendering.Uwp.AdaptiveSubmitAction submitAction)
            {
                // If it's one of the system actions
                if (submitAction.AdditionalProperties.TryGetValue("-ms-systemActivationType", out IJsonValue systemJsonValue) && systemJsonValue.ValueType == JsonValueType.String)
                {
                    switch (systemJsonValue.GetString())
                    {
                        case "dismiss":
                            TriggerActivation("dismiss", ActivationType.System);
                            return;

                        case "snooze":
                            TriggerActivation("snooze", ActivationType.System);
                            return;
                    }
                }

                // Utilize the Data property
                ...
                try
                {
                    ...
                    if (submitAction.AdditionalProperties.TryGetValue("-ms-activationType", out IJsonValue activationTypeJsonValue))
                    {
                        if (activationTypeJsonValue.ValueType == JsonValueType.String)
                        {
                            switch (activationTypeJsonValue.GetString().ToLower())
                            {
                                case "background":
                ...
                {
                    arguments = ex.ToString();
                }

                TriggerActivation(arguments, activationType);
            }

            else if (action is AdaptiveCards.Rendering.Uwp.AdaptiveOpenUrlAction openUrlAction)
            {
                try
                {
                    // Try/catch getting the Url property since if it's a bad/invalid Uri, it'll throw an exception since the Uri object
                    // is created when you call get_Url
                    TriggerActivation(openUrlAction.Url.ToString(), ActivationType.Protocol);
                }
                catch (Exception ex)
                {
                    TriggerActivation(ex.ToString(), ActivationType.Foreground);
                }

            }
        }

        private void Renderer_Action(AdaptiveCards.Rendering.Uwp.RenderedAdaptiveCard sender, AdaptiveCards.Rendering.Uwp.AdaptiveActionEventArgs args)
        {
            TriggerCardActivation(args.Action);
        }
#endif
                ...
                if (toastContent.Visual != null)
                {
                    ...
                    if (binding != null)
                    {
                        ...
                        if (appLogoOverride != null)
                        {
                            ...
                            // AppLogoOverride images will become 48px large
                            if (CurrFeatureSet.RS1_Style_Toasts)
                            {
                                GridAppLogo.Width = 48;
                                GridAppLogo.Height = 48;
        ...
        private ValueSet GetCurrentUserInput()
        {
            ...
            {
                return _renderedAdaptiveCard.UserInputs.AsValueSet();
            }
#endif
        ...
        private void ToastMainContainerButton_Click(object sender, RoutedEventArgs e)
        {
            if (IsCardToast())
            {
                TriggerCardActivation(_selectAction);
            }
        ...
        private bool IsCardToast()
        {
            return _currJsonContent != null;
        }
...
#region DeviceFamily
        ...
        private void thisElement_DragStarting(UIElement sender, DragStartingEventArgs args)
        {
            args.AllowedOperations = Windows.ApplicationModel.DataTransfer.DataPackageOperation.Copy;

            if (_currJsonContent == null)
            {
                args.Cancel = true;
                return;
            }
            ...
            if (_currJsonContent.Images?.Card != null)
            {
                dynamic img = new ExpandoObject();
                img.type = "Image";
                img.size = "stretch";
                img.url = _currJsonContent.Images.Card.Url;
                body.Add(img);
            }

            List<dynamic> textElements = new List<dynamic>();
            if (_currJsonContent.Title != null)
            {
                textElements.Add(JsonToastTextToAdaptiveText(_currJsonContent.Title, false, true));
            }
            if (_currJsonContent.Subtitle != null)
            {
                textElements.Add(JsonToastTextToAdaptiveText(_currJsonContent.Subtitle, true));
            }

            if (_currJsonContent.Images?.Profile != null)
            {
                dynamic columnSet = new ExpandoObject();
                columnSet.type = "ColumnSet";

                List<dynamic> columns = new List<dynamic>();

                dynamic appLogo = new ExpandoObject();
                appLogo.type = "Image";
                appLogo.size = "stretch";
                if (_currJsonContent.Images.Profile.Style == JsonToastAppLogoImageStyle.Person)
                {
                    appLogo.style = "person";
                }
                appLogo.url = _currJsonContent.Images.Profile.Url;

                dynamic colAppLogo = new ExpandoObject();
                colAppLogo.type = "Column";
                colAppLogo.size = "1";
                colAppLogo.items = new dynamic[]
                {
                    appLogo
                };

                columns.Add(colAppLogo);

                dynamic colTextElements = new ExpandoObject();
                colTextElements.type = "Column";
                colTextElements.size = "8";
                colTextElements.items = textElements;
                columns.Add(colTextElements);
            ...
            {
                body.AddRange(textElements);
            }

            if (_finalCardJson != null)
            {
                dynamic finalCard = JsonConvert.DeserializeObject<ExpandoObject>(_finalCardJson);
                if (JsonParser.HasProperty(finalCard, "body"))
                {
                    foreach (var b in finalCard.body)
                    {
                        body.Add(b);
                    }
                }
                if (JsonParser.HasProperty(finalCard, "actions"))
                {
                    foreach (var a in finalCard.actions)
                    {
                        actions.Add(a);
                    }
                }
        ...
        private static dynamic JsonToastTextToAdaptiveText(JsonToastTextBlock text, bool noSpacing, bool isTitle = false)
        {
            dynamic answer = new ExpandoObject();
            answer.type = "TextBlock";
            ...
            if (isTitle)
            {
                answer.weight = "bolder";
                answer.maxLines = 2;
            }
            else
            {
                answer.isSubtle = true;
                answer.maxLines = 4;
            }
